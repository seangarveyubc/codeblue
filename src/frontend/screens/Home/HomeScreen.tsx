        ...
        <View style={styles.container}>
            ...
            <View style={styles.devicesContainer}>
                <View style={styles.deviceHeader}>
                    ...
                    <Text style={styles.edit} onPress={toggleChecked}>{deviceListState ? 'Edit' : 'Save'}</Text>
                </View>
                {!bluetoothState ? (
                    <View style={styles.bluetoothPrompt}>
                        <CentredContent>
                            <Text>CodeBlue requires Bluetooth to monitor heart rate.
                                <Text style={{color:Colours.BLUE}}>Turn on Bluetooth.</Text>
                            </Text>
                        </CentredContent>
                    </View>
                ) : null}
                {deviceListState ? (
                    <View style={{flex:bluetoothState ? 7:6, marginTop: 10}}>
                        <CentredContent>
                            <DeviceWidget name={deviceName1} isConnected={true}/>
                            <DeviceWidget name={deviceName2} isConnected={false}/>
                        </CentredContent>
                    </View>
                ) : (
                    <View style={{flex:bluetoothState ? 7:6, marginTop: 10}}>
                        <CentredContent>
                            <IconTextInput text={deviceName1} isConnected={true} onChangeText={text => changeDeviceName1(text)}/>
                            <IconTextInput text={deviceName2} isConnected={false} onChangeText={text => changeDeviceName2(text)}/>
                        </CentredContent>
                    </View>
                )}
            </View>
